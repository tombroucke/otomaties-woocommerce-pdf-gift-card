        ...
        foreach ($order->get_items() as $key => $item) {
            ...
            for ($i=0; $i < $item->get_quantity(); $i++) {
                if (apply_filters('gc_create_coupon', true, $item) && ! wc_get_order_item_meta($key, '_gc_coupon_' . $i, true)) {
                    $coupon_code = strtolower(str_pad(substr(wc_get_order_item_meta($key, '_gc_recipient', true), 0, 5), 5, 'X') . '_' . substr(md5(uniqid((string)rand(), true)), 0, 5));
                    ...
                    $item->add_meta_data('_gc_coupon_' . $i, (string)$new_coupon_id);
